...
Crawler.prototype.crawl = function () {
    ...
    async.eachSeries(config.selector, function (item, callback) {
        ...
        if (index === config.selector.length - 1) {
            if (config.type) {
                if (that[config.type]) {
                    if(urlLevels[index - 1] && urlLevels[index - 1].length){
                        that[config.type](urlLevels[index - 1]);
                    }else{
                        that.log('未抓取到任何能容，可能是选择器书写有错', 'redBG');
                    }
                } else {
...
Crawler.prototype.image = function (urls) {
    ...
    async.whilst(function () {
      ...
      //                   consloe.log("url = "+url)
                    ...
                    // });
                // }
                that.log('第 {0} 套图片收集了{1}张图片'.format((i + 1) + '/' + count, (i + 1)));
        ...
        mkdirp(filepath, function (err) {
            if (err) {
                ...
                request.head(item.url, function (err, res, body) {
                    ...
                    var savePath = path.join(filepath, decodeURIComponent(filename));
